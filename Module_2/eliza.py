...
THIS FILE CAN BE RUN ON ITS OWN: type: python3 eliza.py   to run it
...
# and we used all of the lecture material as well
...
def talk_to_me():
    ...
    name = input('>')
    print(f'Eliza: Hi {name}. How can I assist you today?')

    while True:
        statement = input(f'{name}: ')
        if statement.lower() in ['i quit', 'i am done', 'good bye']:
            print(f'Goodbye, {name}. It was nice talking to you.')
            break
...
"""
This line checks if the script is being run directly by the Python interpreter (as opposed to being imported as a module).
If the script is run directly (i.e., __name__ is set to "__main__"), the talk_to_me function is called to start the program.
This is a common idiom in Python programs to prevent code from being run when the module is imported.
"""
